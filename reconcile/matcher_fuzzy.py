...
def _ensure_credit_convention_resolved(
    bank: pd.DataFrame, ledger: pd.DataFrame, invert_credit_convention: bool | str = "auto"
) -> None:
    ...
    This lets shortlist_candidates() be called standalone (fuzzy-only
    pipelines / tests) while still being a no-op, not a double-invert,
    when it runs after run_exact_matching on the same dataframes."""
...
def shortlist_candidates(
    ...
    for _, b in bank[~bank["matched"]].iterrows():
        ...
        # Reference match (if any) outranks pure text similarity; within
        # each of those two buckets, higher description score wins.
